...
Lib.Card {
  ...
  // ---------- Native Model ----------
  // Passed in from shell.qml via NotificationBubble
  property var historyModel: null
  property var notifModel: historyModel ? historyModel.model : null
  ...
  function dismissOne(index, id) {
    if (historyModel) historyModel.removeAt(index)
  }
  ...
  SequentialAnimation {
    ...
    ScriptAction {
      script: {
        if (historyModel) historyModel.clearAll()
      }
    }
  ...
  ColumnLayout {
    ...
    RowLayout {
      ...
      Rectangle {
        ...
        Text {
          ...
          text: notifModel ? String(notifModel.count) : "0"
          font.family: engine.textFont
          font.pixelSize: 11
          font.weight: 900
          color: root.cFgMain
    ...
    Text {
      visible: (!notifModel || notifModel.count === 0) && (!root.compactMode || root.expanded)
      opacity: visible ? 1 : 0
    ...
    Item {
      ...
      ListView {
        ...
        delegate: Lib.NotifItem {
          width: list.width
          height: model.body ? listWrapper.itemH + 16 : listWrapper.itemH
          
          nId: model.notifId !== undefined ? model.notifId : 0
          app: model.appName !== undefined ? model.appName : "Notification"
          summary: model.summary !== undefined ? model.summary : ""
          body: model.body !== undefined ? model.body : ""
          image: model.image !== undefined ? model.image : ""
          time: model.time !== undefined ? model.time : ""
          theme: root.engine
          
          onClicked: root.dismissOne(index, model.notifId)
        }
      }
